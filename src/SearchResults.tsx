        ...
        <div className="song" onClick={() => dispatch(track(trackItem.id))}>
          ...
          <div className="trackInfo albumTrack">
            ...
            {trackItem.artists.length > 0 && trackItem.artists[0].name && (
              <div>
                {trackItem.artists.length === 1 ? <p className='statDescription'>Artist</p>: <p className='statDescription'>Artists</p>}
                <p>{trackItem.artists.map((artist, index) => <a href={artist.external_urls.spotify} target='_blank' rel="noreferrer">{artist.name}{ trackItem.artists.length - 1 === index ? '' : ', '}</a>)}</p>
              </div>
            )}
          </div>
        </div>
    ...
    if (queryMatches !== undefined){
        return (
            <div id="searchResults">
                {queryMatches.map((match) => <Song key={match.id} trackItem={match} imageHeight={imageHeight} />)}
            </div>
        )
    }
}